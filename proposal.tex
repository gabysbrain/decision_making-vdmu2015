\tmnote{abstract is blank?}
\tmnote{Title is weird: ``in the visualization''?}
...
%
\msnote{agree, cite Tory as well (although I would not claim that you guys were the first ever to talk about mental models :-), you could also cite the TOI paper here, imho this is exactly what we try to do} 
%
...
\msnote{list more ...}, and people are known to adapt strategies based on ... \msnote{extend and cite}.
...
In this position paper, we seek to conduct some first steps towards such a more sophisticated understanding of the role of users in 
...
directly supports the evaluation of numerical weights on objectives. \tmnote{But Tuner does too.}
...
also set thresholds and filter out the unacceptable decisions. \tmnote{rather than filtering ... filtering?!?!} This process
is repeated until a final choice remains. This process is called
the majority of confirming dimensions strategy. \tmnote{huh? its called elimination by aspects!?}
...
We did not see any evidence in the visualization \tmnote{Why do you keep saying 'in the visualization'???  this is not proper english?!} of this strategy being
...
Lexicographic and elimination by aspects are the most popular heuristics
by far. Both of these concentrate on one parameter at a time and are
about filtering alternatives. \msnote{add a bit more in terms of summary here, if possible}
...
\tmnote{Needs some thought. I don't think it is good to ask whether the vis communicity is already doing this. Rather, how can the current approaches of task taxonomies and mantras be reconciled with work in the problem solving / decision making world. What are the similarities, what are the differences?}
...
\tmnote{again, I would rephrase towards 'how can we support *different* decision making / problem solving strategies'?}
...
\tmnote{confusing -- is paraglide now the only tool or not?}
...
make while evaluating uncertainty. \tmnote{rephrase, perhaps: we are proposing to study more closely the impact that uncertainty information has on decisions and the implications for visual tool design. or sth like that.} We have enumerated some common decision
