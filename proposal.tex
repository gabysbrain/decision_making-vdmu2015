...
\msnote{methodology matters - mcgrath} or
...
methodology to study/analyse and support~\ttwwarning{ref}.

On the other hand, design studies are highly focused on a particular
application scenario~\ttwwarning{ref}. While not as generalizable, the end goal of a design
...
\emph{generalizable} to the larger populace~\ttwwarning{ref}. \msnote{we hypothesize?} 
...
with task taxonomies~\ttwwarning{cite Bremer and another one} or 
models~\ttwwarning{cite Keim and another}. These are largely focused on 
``what'' users are doing though and not
``why'' they are doing it (even Bremer) 
\tmnote{call out bremer}\msnote{is this all that's missing?}. What is
...
understanding \msnote{vague...}. In addition, better user modeling will reduce the number of
...
how those correspond to visualizations~\ttwwarning{ref}. While this does make the visualization
...
multi-step activities''~\citep{Frensch:2005}. Within this framework\msnote{???} is insight
...
and ``knowledge construction''~\ttwerror{ref}. Even with these definitions though
...
measure the aquisition of insights \tmnote{sharpen}.
...
\tmnote{model behind it?}. To this end, we should look at the work that
...
double check the exact definition) \tmnote{rewrite}.
...
in visualization\tmnote{huh?} and implications for design of visualization
...
\tmnote{will it improve vis tools and the (visual) design (study) process?}
...
and different optimal selections will be made~\ttwerror{ref}.
...
lexicographic~\ttwerror{ref} by the decision making community.
...
the majority of confirming dimensions~\ttwerror{ref} strategy. 
...
Vismon~\ttwerror{ref} also uses a similar strategy.
...
been called the frequency of good and bad features~\ttwerror{ref}.
...
Satisficing~\ttwerror{ref} does not consider the entire set of decision options
...
will influence which decision making strategy one employs~\ttwerror{ref}.
