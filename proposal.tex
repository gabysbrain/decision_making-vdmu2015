...
(the same set as Sedlmair et al.~\citep{Sedlmair:2014}). We classified these papers based on which strategy 
type, if any, was employed by the users of the tools when describing the task analysis, user characterization,
or case study. We found that $15/21$ of the papers described
...
The other strategies were rarer. For instance, only one of the papers examined in Sedlmair et al.~\cite{Sedlmair:2014},
ParaGlide~\citep{Bergner:2013} concentrated on a
weighted strategy \msnote{in the table there are three, needs to be fixed or re-phrased}. ParaGlide did this through computing and
...
tools we examined \msnote{given that it is so cool for vis ... (say why it would be nice to have more)}. As far as we know, there is only 
very little work on visualization tools that allows the user to directly see the
effect of manipulating weights, such as LineUp~\citep{Gratzl:2013}, or ValueCharts \msnote{add reference}.
\msnote{Yes, good to have LineUp here and not in Section 2.}
\ttwnote{what else...}
...
solve a problem.
We believe that with better understanding of decision making processes used one could better decide which tasks to prioritize for which users.
We propose that linking task taxonomies with the different problem 
solving strategies in terms of which strategies are supported by which 
...
tool. \msnote{maybe break down into 2 sentences?}
